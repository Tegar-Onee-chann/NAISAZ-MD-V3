...
module.exports = {
    ...
    noLimit: true,
    start: async(naisa, m, { mime, quoted, text, prefix, command }) => {
        if (/image/.test(mime)) {
            let media = await naisa.downloadAndSaveMediaMessage(quoted)
            if (text.toLowerCase() === "original") {
                ...
                await naisa.updateProfilePicture(m.from, { url: media })
                .then( res => {
                    m.reply("Sukses")
                    fs.unlinkSync(media)
                }).catch(() => m.reply('err'))
